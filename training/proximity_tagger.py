...
def phrase_analysis(filepath):
    ...
    for i in range(len(sentenceList)-1):
        ...
        lpcount=lpcount+1
        print '\tPhrase Analysis Sub Loop : ',lpcount,'.'*10,(float(lpcount)*100/float(totalcount)),'%'
...
def medianlist(filename,isphrase,cnt_var,isneg,pathname):


    if isphrase:
        median_testset=[]
        f = open(pathname+'\phrase_analysis_part_file','r')
        median_testset=json.load(f)
        f.close()    
        newlist=median_testset[isneg][cnt_var]
    else:
...
def bin_list(filename,isphrase,cnt_var,isneg,pathname):
...
def pattern_list(filename,isphrase,cnt_var,isneg,pathname):
