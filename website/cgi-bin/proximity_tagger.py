	...
	#print 'Word is : ', word[0]
	    ...
	    #print '\nWord is : ', word[0]
            if word in positive or word in negative:
                ...
                if word in negative:
                    pol=-1
		    print 'Word is in Negative'     
                newlist.append((i,pol))
		#print 'Word is in Positive'
            else:
		    ...
		    #print 'Values  : ' , dist.pos_score, dist.neg_score, val
                    if val!=0.0:
...
def phrase_polarity(tree_string):
	...
	i=tree_string.rfind('(')
		...
		#print 'Phrase Polarity \n'
        	#print 'Positive list : ' , positive
                #print 'Negative List : ' , negative
		#print 'word is : ', word_str
      		#print 'Values  : ' , dist.pos_score, dist.neg_score, repl_val
...
def phrase_analysis(filepath):
	...
	bracketed=os.popen("stanford-parser/lexparser.sh "+filepath).readlines()        
    ...
    temp_file2= open('tempFile.txt','w')
    temp_file2.write(cnvrtd_line)
    temp_file2.close()
    tag_words = []
    tag_words = taglist('tempFile.txt')
...
def medianlist(filename,isphrase):
	...
	#print '\nFirst pp : ', pp_distance_list
   	#print 'First nn : ', nn_distance_list
    	#print 'Forst pn : ', pn_distance_list
	...
	ppval = 0
   	...
   	ppval = numpy.median(ppmedlist)
    if nnflag==0:
        nnval = 0
    else:
        nnval = numpy.median(nnmedlist)
    if pnflag==0:
        pnval = 0
    else:
        pnval = numpy.median(pnmedlist)

    #elt=(numpy.median(ppmedlist),numpy.median(nnmedlist),numpy.median(pnmedlist))     
    elt = (ppval,nnval,pnval)
    #print '\nelt : ' , elt
    return elt
